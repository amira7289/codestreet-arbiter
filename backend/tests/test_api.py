...
def offer(client, case_id, **kw):
    ...
    payload.update(kw)
    return client.post(f"/cases/{case_id}/offers", json=payload)
...
def test_accepted_offer_settles_without_a_verdict(client):
    ...
    responded = client.post(f"/cases/{case['id']}/offers/{made.json()['id']}/respond",
                            json={"action": "accept"})
...
def test_settled_case_cannot_be_adjudicated(client):
    ...
    client.post(f"/cases/{case['id']}/offers/{made.json()['id']}/respond", json={"action": "accept"})

    assert client.post(f"/cases/{case['id']}/resolve").status_code == 409
...
def test_settlement_withdraws_a_standing_verdict(client):
    ...
    client.post(f"/cases/{case['id']}/gather")
    client.post(f"/cases/{case['id']}/resolve")
    assert client.get(f"/cases/{case['id']}").json()["verdict"] is not None
...
def test_counter_offer_supersedes_the_open_one(client):
    ...
    offers = {o["id"]: o for o in client.get(f"/cases/{case['id']}").json()["offers"]}
...
def test_declining_returns_the_case_to_negotiating(client):
    case = make_case(client, transaction_id="TX1001")
    made = offer(client, case["id"])
    client.post(f"/cases/{case['id']}/offers/{made.json()['id']}/respond", json={"action": "decline"})
    ...
    assert client.post(f"/cases/{case['id']}/resolve").status_code == 200
...
def test_an_answered_offer_cannot_be_answered_again(client):
    case = make_case(client, transaction_id="TX1001")
    made = offer(client, case["id"])
    path = f"/cases/{case['id']}/offers/{made.json()['id']}/respond"
...
def test_forecast_is_read_only_and_identical_for_both_parties(client):
    ...
    first = client.get(f"/cases/{case['id']}/forecast").json()
    second = client.get(f"/cases/{case['id']}/forecast").json()
